...
### gai 能力族登记（2026-09-09）

`gai` 及其子包属于能力层，同族子包允许单向引用但不得形成循环依赖；核心不得 import 其他能力族。
与其他能力包的组合通过接口注入或 `gai/adapters/<实现名>` 完成，适配器遵循第 3.3 节。
`scripts/check-deps.sh` 已将 `gai` 纳入能力族检查；具体适配器落地时须同步完善检查中的适配层识别。
