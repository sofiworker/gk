...
export default function () {
  ...
  const panicRes = http.get(`${base}/fault/panic`, { headers: { 'X-Test-Namespace': namespace }, tags: tags('error', 'panic_recovery', 500) });
  check(panicRes, { 'error panic recovery status 500': (r) => r.status === 500 });
  ...
  check(rs, {
    'security not found status 404':r=>r[0].status===404, 'security not found json body':r=>{const v=r[0].json();return v.error&&typeof v.error.message==='string'},
