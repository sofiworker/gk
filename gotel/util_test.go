...
func TestUtil(t *testing.T) {
	...
	if toString(int64(123)) != "123" {
		t.Error("int64 failed")
	}
	if toString(12.34) != "12.34" {
		t.Error("float64 failed")
	}
	if toString(true) != "true" {
		t.Error("bool failed")
	}
	if toString(nil) != "null" {
		t.Error("nil failed")
	}

	// Test convertStatusCode
	if convertStatusCode(StatusCodeOk) != codes.Ok {
		t.Error("ok failed")
	}
	if convertStatusCode(StatusCodeError) != codes.Error {
		t.Error("error failed")
	}
	if convertStatusCode(StatusCodeUnset) != codes.Unset {
