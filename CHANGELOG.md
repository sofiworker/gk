...
- 仓库：**golangci-lint 从 v2.4.0 升到 v2.13.2**——v2.4.0 与 Go 1.27 工具链不兼容（unified export data 变化使 typecheck 解析不了嵌入字段），会对现有代码报假阳性（实测 `ghttp` 包 9 条 `req.URL undefined`，而 `go vet` 通过），且 `//go:build go1.27` 文件在 Go 1.25 的 lint job 下被构建标签排除、**从未被 lint 覆盖**。现 CI 的 lint job 改在 Go 1.27 上运行，`.golangci.yml` 补 `issues.max-same-issues: 0`（默认值 3 会让同类告警被静默截断，造成"修完仍不绿"的循环）；一并清理全仓既有 lint 问题 95 条（ghttp 66 + 其他能力包 29），当前 `golangci-lint run ./...` 为 **0 issues**。CI 的 `preview`/`fmt` job 亦从 `GOTOOLCHAIN=go1.27rc2` 改为正式版 `go1.27.1`。
...
- ghttp：**路径校验层拦掉控制字符与 DEL（400）**。校验作用于【解码后】路径，`%00`/`%0a` 正是由此进入内部逻辑：前者变成静态层 500，后者原样进访问日志与 panic 日志，凭空伪造整行记录。在唯一入口拦截使下游各层无需各自设防；RFC 3986 本就不允许路径出现裸控制字节。
- ghttp：**请求体上限内建到解码层**。`LimitBody` 是 opt-in 中间件，未挂载时非 POST/PUT/PATCH 方法（标准库不为其读体，由本包 `io.ReadAll`）与 `textCodec`（须整块读入内存）完全无上限，一条超大体即可打满内存；原注释把 opt-in 当成了默认存在。现统一经 `readAllCapped`（表单 10 MiB，与标准库 `ParseForm` 同阶；text 10 MiB），超限返回 413 且不被包成 400。
- ghttp：**WebSocket 默认设读上限**。标准库封装从不 `SetReadLimit` 且无任何设置点，任意已连接客户端都能用持续大帧耗尽服务端内存。`NewWSUpgrader` 现默认 1 MiB，新增 `WithWSReadLimit`（传 0 或负值显式退回不限）。
- ghttp：**Timeout 的 deadline 现在真正到达 handler**。过去只作为闭包参数下传，而 raw 端点普遍读 `req.Request.Context()`，看到的是无 deadline 的父 ctx——「已超时」的请求仍能无限阻塞，同时中间件已写出 504，一个请求里存在两个互相矛盾的上下文。现改写 `req.Request` 并在返回时复原（池化安全）。另修「吞掉下游 cause」：原先无条件返回 `ErrRequestTimeout`，排障只剩「超时」二字；现在 `timeoutError` 同时保住超时与 cause，并靠 StatusCoder 让 504 不被 cause 的分类顶掉。
- ghttp：**`ClientIP()` 合并转发头的全部行**。`Header.Get` 只取第一行，而代理常以「再写一行 X-Forwarded-For」追加链路：客户端预置一行、代理再补一行时，只看第一行等于让客户端那一行单独成立，整条可信链路被忽略，攻击者地址成为 ClientIP，按 IP 限流与审计双双失效。
- ghttp：**gzip 两处边界**。① 206 或带 `Content-Range` 的响应不再压缩——该头描述原始字节区间，压缩后同一区间对应不同长度，客户端解压后彻底对不上。② 未定案就 `Flush` 会先提交不带 `Content-Encoding` 的 200 头，随后的 Write 又写入 gzip 字节，客户端拿到「没有头的 gzip 体」；现 `Flush` 显式完成压缩决策后再冲刷。
- ghttp：**注册期方法校验与服务启动后的注册守卫**。① Go 原样透传客户端方法，注册 `"get"` 永远匹配不到 GET 却会建出一棵树，并使 `MatchRoute` 对真实请求返回空串，观测层静默丢失路由归属；现限制方法为合法 token 并拒绝标准方法的小写变体（自定义扩展仍允许）。② 路由树是无锁读结构，服务开始后注册与匹配构成数据竞争（race detector 判死）却长期「看起来能跑」；现返回 `ErrRegistrationAfterStart`。
- ghttp：**Recovery 不再吞掉已提交响应的 panic，且 500 走统一错误链**。原先无论是否已提交都 `err = nil`，于是流式场景（SSE、大文件、已 Flush）中途崩溃的请求在 onError 钩子、访问日志与 5xx 指标中彻底消失——客户端拿到被截断的 200，服务端记录「一切正常」。现一律上抛 `panicError`，已提交时 `writeError` 只如实上报而不改写响应；同时不再自写 text/plain 500（包内最后一处绕过错误链的出口）。
- ghttp：**日志字段净化，堵住日志注入**。路径层的控制字符拦截覆盖不到【查询串】及由它派生的错误文案：`?name=x%0aFORGED` 会原样出现在 `err.Error()` 里，而日志按行解析时一个裸换行足以凭空造出整条记录（伪造「admin login ok」这类事件）。新增 `sanitizeLogToken`，接入 Logger 与 Recovery 的默认 sink；只消除折行能力，不做脱敏。
- ghttp：**限流三处失效模式**。① `RPS <= 0` 对 NaN 恒为 false，NaN 会穿过校验使桶容量与补充速率都成为 NaN、`tokens < 1` 永假，每个请求都被放行而限流【静默】失效；现用 `!(RPS > 0)`。② 回收窗口不短于空桶补满时间 `burst/RPS`，否则「仍持有未用令牌的桶」被删除后按新 key 重建为满桶，每个 idle 周期白得一次完整 burst（RPS=1/Burst=1000/Idle=1s 可长期跑到 ~500 RPS）。③ 默认维度不再可能返回空 key（空 key 的语义是「显式豁免」），取不到地址时落入共享兜底桶；自定义 `KeyFunc` 返回空串的豁免语义不变。
- ghttp：**CORS 三处修正**。① 来源比对大小写不敏感（Origin 只含 scheme://host[:port]，按 URL 语义均不区分大小写），与 CSRF 同一口径，避免两道防线对「是否同源」判断分叉。② 显式白名单【优先】于通配：配 `["*", "https://a.example"]` 时该来源必须回显自身而非被通配写成 `*`。③ `Vary: Origin` 改为幂等追加（重复项虽合法，却使下游缓存键解析做无谓工作并读成多条策略）。
- ghttp：**CSRF 两处**。① 新增 `CSRFConfig.TrustedOriginsOnly`：开启后不再以「Origin 的 host 等于请求 Host」作为同源兜底——Host 属客户端可影响输入，通配虚拟主机或 DNS 重绑定下 `Host: evil.com` 与 `Origin: http://evil.com` 成对出现即可绕过白名单。默认关闭以保持既有部署可用（本站自身通常靠该兜底放行，强制开启会让同源表单提交全部 403），对来源有硬要求时显式收紧。② 取表单 token 前先按声明长度设门（4 MiB）：为一个注定缺 token 的请求解析超大 multipart 体会先吃掉内存与临时文件（`ParseMultipartForm` 的 32 MiB 只限驻留内存而非总大小）之后才拒绝，攻击者可用注定失败的请求消耗资源。
